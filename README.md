...
[**Lit**](https://lit.dev/) and native [**Web Components**](https://developer.mozilla.org/en-US/docs/Web/API/Web_components) using [**Serenity/JS**](https://serenity-js.org) and the [**Playwright Test**](https://playwright.dev/) runner with [**Playwright CT Web**](https://github.com/sand4rt/playwright-ct-web) extension,
...
- Pre-configured **Serenity/JS** + **Playwright Component Test** + **Web Components** + **Lit** + **TypeScript** setup
...
│   │   ├── Dropdown.serenity.ts     # Component test scenarios + Serenity/JS page elements
...
playwright.config.ts     # Playwright Component Test configuration
...
- `*.spec.ts` - Test scenarios using Serenity/JS and Playwright CT
...
- **Write test scenarios**: Add `*.serenity.ts` files alongside your components to test them in isolation
...
Ensure your component exports are correct in the `index.ts` files and that you're importing from the right path in your test files.
